...
int fs_mkdir(const char *pathname, mode_t mode)
{
    ...
    DE *emptyDE = lookForFreeDE(info->parentDirPtr);
    if (emptyDE == NULL)
    {
        printf("\ndirectory full\n");
        return -1;
    ...
    if (strcmp(info->path, currPath) == 0)
    {
        ...
        int retReload = loadUpdatedDir(updatedParentDir);
...
fdDir *fs_opendir(const char *pathname)
{
    ...
    else if (fileOrDirVal == 1)
    {
        ...
        int numBlocks = totalNumOfBlocks((totDirEnt * sizeof(DE)));
...
int fs_rmdir(const char *pathname)
{
    ...
    freeBlocks(parseInfo->parentDirPtr[parseInfo->index].location, totalNumOfBlocks(parseInfo->parentDirPtr[parseInfo->index].fileSize));
    ...
    dirToDisk(parseInfo->parentDirPtr);

    if (strcpy(parseInfo->path, currPath) == 0)
    {

        loadUpdatedDir(parseInfo->parentDirPtr);
    }
    printf("Removed given directory\n");
    return 0;
}
...
int fs_delete(char *filename)
{
    ...
    // Use freeTheBlocks(location, totalNumOfBlocks(size of file))
    freeBlocks(parseInfo->parentDirPtr[parseInfo->index].location, totalNumOfBlocks(parseInfo->parentDirPtr[parseInfo->index].fileSize));
