...
b_io_fd b_open(char *filename, int flags)
{
	...
	PP *pathInfo = parsePath(filename);
	...
	if (pathInfo->exists != 0 && (flags & O_CREAT) == O_CREAT)
	{
		...
		fcb->parentDir = malloc(totDirEnt * sizeof(DE));
...
int b_write(b_io_fd fd, char *buffer, int count)
{
	...
	if (fcbArray[fd].fileDE->fileSize >= fcbArray[fd].fileDE->dirBlocks * BLOCK_SIZE)
	{
		...
		char *newBuf = malloc(fcbArray[fd].fileDE->dirBlocks * BLOCK_SIZE);
		...
		freeBlocks(fcbArray[fd].fileDE->location, fcbArray[fd].fileDE->dirBlocks);
...
int b_read(b_io_fd fd, char *buffer, int count)
{
	...
	if (*bytesNotCopied > 0)
	{
		...
		int copyBytes = (*bytesNotCopied > bytesToCopy) ? bytesToCopy : *bytesNotCopied;
